...
# My read function for loading masked input and their corresponding output
def read_masked_examples(filename, data_num):
    """Read masked training examples from a JSON file."""
    examples = []
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        for idx, entry in enumerate(data):
            examples.append(
                Example(
                    idx=idx,
                    source=entry["input"].strip(),
                    target=entry["output"].strip()
                )
            )
            if idx + 1 == data_num:
                break
